...
namespace DialogueQuests
{
    ...
    public class NarrativeControls : MonoBehaviour
    {
        ...
        private void Start()
        {
            journal = FindObjectOfType<QuestPanel>().gameObject;
        }

        void Update()
        {
            if (!chatButton.activeInHierarchy)
            {
        ...
        public void JournalUse()
        {
            ...
            if(journalopen)
            {
                journal.GetComponent<QuestPanel>().ShowPanel();
            }    
            else
            {
                journal.GetComponent<QuestPanel>().HidePanel();
            }
        }
